...
The `home-routes` handler in the `graph-ql-clojure.routes.home` namespace
...
The `home-page` function will in turn call the `graph-ql-clojure.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `graph-ql-clojure.handler` namespace:
...
Request middleware functions are located under the `graph-ql-clojure.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `graph-ql-clojure.dev-middleware` namespace found in
